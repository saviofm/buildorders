...
class CFKeyProvider {
    async getCfSpaceInfo() {
        ...
        if (!(orgs && orgs.resources.length === 1)) {
            throw new CliError(`CF org ${org} not found!`);
        }
        ...
        if (!(spaces && spaces.resources.length === 1)) {
            throw new CliError(`CF space ${space} not found in org ${org}!`);
        }
    ...
    async resolve(name, binding) {
        ...
        try {
            ...
            throw new CliError(`Service instance "${instance}" not found.`);
        }
        switch (instanceObj.type) {
            ...
            default:
                throw new CliError(`Service type ${instanceObj.type} not supported`)
        }
    }
    ...
    async executeRequest() {
        ...
        for (const filter of filters) {
            ...
            if (filteredResources.length === 1) {
                ...
                filter.reject(new CliError(`Couldn't find resource for url "${this.url}" with filter "${fmt(filter.filter)}"`));
            } else {
                filter.reject(new CliError(`Found more than one resources for url "${this.url}" with filter "${fmt(filter.filter)}"`));
            }
        }
    }
