...
const { hasJavaNature, getDefaultModelOptions, BuildError, hasOptionValue } = require('../util')
...
class BuildTaskProviderInternal extends BuildTaskProvider {
    ...
    async _createTasks(tasks, addRequiredTasks) {
        ...
        if (addRequiredTasks) {
            ...
            const mtxTask = this._createMtxTask(srv, tasks)
            if (mtxTask) {
                tasks.push(mtxTask)
            }
        }
    ...
    _createMtxTask(src, tasks) {
        ...
        if (BuildTaskProviderInternal._isMtxs()) {
            ...
            if (!tasks.some(task => task.for === BUILD_TASK_JAVA || task.for === BUILD_TASK_JAVA_CF)) {
                this.logger.debug("MTX app without sidecar")
                return {
                    for: BUILD_TASK_MTX,
                    src
                }
    ...
    static _setDefaultSrcFolder(task) {
        switch (task.for) {
            ...
            case BUILD_TASK_MTX_EXTENSION:
                ...
                break
            case BUILD_TASK_MTX:
                require('../../util/version').checkMtx(cds)
                // mtxs with nodejs, but without sidecar
                task.src = task.src || this._normalizePath(cds.env.folders.srv)
                break
            default:
    ...
    static _setDefaultModel(task) {
        ...
        if (this._hasJavaNature(cds.env.folders.srv)) {
