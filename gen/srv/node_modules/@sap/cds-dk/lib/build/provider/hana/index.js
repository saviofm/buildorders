...
const { OUTPUT_MODE_NONE, OUTPUT_MODE, CONTENT_PACKAGE_JSON, CONTENT_HDBTABLEDATA, CSV_FILE_DETECTION,
    CONTENT_ENV, CONTENT_DEFAULT_ENV_JSON, CONTENT_NODE_MODULES, OUTPUT_MODE_RESULT } = require('../../constants')
const { ERROR, WARNING } = BuildTaskHandlerInternal
...
class HanaModuleBuilder extends BuildTaskHandlerInternal {
    ...
    async _writePackageJson() {
        ...
        if (exists) {
            this.logger._debug && this.logger.debug(`skip create [${relativePaths(cds.root, packageJson)}], already existing`)
        }
            ...
            await Promise.all(migrationTableFiles.map(async file => {
                try {
                    ...
                    if (/^>>>>>/m.test(tableModel.migrations.toString())) {
                        ...
                        this.pushMessage(`Current model changes require manual resolution. See migration file ${path.relative(cds.root, file)} for further details.`, ERROR)
                    }
                } catch (e) {
                    throw new Error(`${path.relative(cds.root, file)}: ${e.toString()}`, ERROR)
                }
            }))
        }
    }
