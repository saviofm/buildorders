...
const { WARNING, ERROR } = NodeCfModuleBuilder
...
class MtxSidecarModuleBuilder extends NodeCfModuleBuilder {
    ...
    async _buildMainApp(sidecarEnv) {
        ...
        const profiles = cds.env.profiles || []
            ...
            if (!profiles.includes("production") && !profiles.includes("prod")) {
                ...
                this.pushMessage(`MTX sidecar build results are created in folder '${main}'. Enable 'production' or 'prod' profile if the folder is configured differently.`, WARNING)
            }
    ...
    _compileSidecarSync(sidecarEnv) {
        ...
        try {
            ...
            this._logger._debug && this._logger.debug(`sidecar model: ${relativePaths(cds.root, modelFilePaths).join(", ")}`)
