...
class FioriAppModuleBuilder extends BuildTaskHandlerEdmx {
    ...
    async prepare() {
        ...
        const originFioriBuildOption = fioriBuildOptions[OUTPUT_MODE]
        fioriBuildOptions[OUTPUT_MODE] = OUTPUT_MODE_RESULT
        ...
        try {
            ...
            for (let [appFolder, appModelGroup] of appModelGroups.entries()) {
                ...
                this.logger._debug && this.logger.debug(`model: ${relativePaths(cds.root, modelPaths).join(", ")}`)
            ...
            fioriBuildOptions[OUTPUT_MODE] = originFioriBuildOption
        }
        return false
    }
    ...
    async _writeEdmxToWebapp(src, dest) {
        ...
        try {
            ...
            this.logger.log(`UI module does not contain a manifest.json [${relativePaths(cds.root, manifestPath)}], skipping build`)
            return
        }
        ...
        if (!mainService) {
            ...
            this.logger._debug && this.logger.debug(`UI module does not have a datasource [mainService], [${relativePaths(cds.root, manifestPath)}], skipping build`)
            return
        }
        ...
        if (!localUri || !uri) {
            this.logger._debug && this.logger.warn(`local uri setting missing for data source [mainService], [${relativePaths(cds.root, manifestPath)}]`)
            return
        }
