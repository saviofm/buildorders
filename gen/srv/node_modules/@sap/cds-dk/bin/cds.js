...
const cli = { //NOSONAR
  ...
  async exec (cmd = process.argv[2], ...argv) {
    ...
    try {
      ...
      const { BuildError } = require('../lib/build')
      const { CliError } = require('../lib/client/helper/errors')
      process.exitCode ??= 1
      if (e.messages?.length) return cds._log(e.messages, { 'log-level': cds.env.log.levels.cli })
      else if (e instanceof CliError || e instanceof BuildError) return console.error(e.message)
  ...
  args (task, argv) {
    ...
    function add_global (k,v='') {
      if (k === '--production') return process.env.CDS_ENV = [...process.env.CDS_ENV?.split(',') ?? [], 'production']
      if (k === '--profile') return process.env.CDS_ENV = [...process.env.CDS_ENV?.split(',') ?? [], ...v.split(',')]
    ...
    process.env.CDS_CONFIG = JSON.stringify(env)
    return [ args, options ]
  },
