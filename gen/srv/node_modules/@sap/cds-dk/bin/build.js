...
async function build([project], options = {}) {
    ...
    options.cli = true // avoid that errors are logged twice
    ...
    if (Number(major) < 6) {
        throw new CliError("This application uses @sap/cds version < 6, which is not compatible with the installed @sap/cds-dk version >= 7. Update @sap/cds to version >= 6.")
    }
    if (options.clean === "false") {
        throw new CliError("cds build option '--clean' is deprecated - build output folders are always cleaned before build")
    }
